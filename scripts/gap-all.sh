...
Optional Arguments:
  ...
  --steps <steps>          Comma-separated list of steps to run (default: all)
                           Available: aws,gcp,ocp,ocm-version-gate,feature-gates,validation
...
if [[ -n "$STEPS" ]]; then
    ...
    for step in "${STEPS_ARRAY[@]}"; do
        ...
        if [[ "$step" != "aws" ]] && [[ "$step" != "gcp" ]] && [[ "$step" != "ocp" ]] && [[ "$step" != "ocm-version-gate" ]] && [[ "$step" != "feature-gates" ]] && [[ "$step" != "validation" ]]; then
            log_error "Invalid step: $step"
            log_error "Valid steps are: aws, gcp, ocp, ocm-version-gate, feature-gates, validation"
            exit 1
        fi
    done
    ...
    STEPS_ARRAY=("aws" "gcp" "ocp" "ocm-version-gate" "feature-gates" "validation")
fi
...
EOF
    ...
    for step in aws gcp ocp validation ocm-version-gate feature-gates; do
